...
public class AsciiDoctorTemplateResolver extends TemplateResolver {
    ...
    private class AdocResourceResolver implements IResourceResolver {
        ...
        public InputStream getResourceAsStream(TemplateProcessingParameters params, String resourceName) {
            try (InputStream line = readInputStreamOrFallbackToEnglish(resourceName, language)) {
				while((line=input.readLine())!=null);
                    ...
                    asciidoctor.convert(new InputStreamReader(line), writer, createAttributes());
        ...
        private InputStream readInputStreamOrFallbackToEnglish(String resourceName, Language language) {
            InputStream line = Thread.currentThread().getContextClassLoader().getResourceAsStream(computeResourceName(resourceName, language.getLocale().getLanguage()));
            if (line == null) {
                line = Thread.currentThread().getContextClassLoader().getResourceAsStream(computeResourceName(resourceName, "en"));
            }
            return line;
